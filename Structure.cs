...
namespace Minecraft
{
    ...
    {
        public static void MakeTree(Vector3i position, Queue<BlockMod> queue, int minHeight, int maxHeight)
        {
            ...
            for (int x = -2; x <= 2; x++)
                for (int y = height - 2; y <= height + 1; y++)
                    for (int z = -2; z <= 2; z++)
                        if ((x != 0 || z != 0) || y >= height)
                            queue.Enqueue(new BlockMod(new Vector3i(position.X + x, position.Y + y, position.Z + z), 16));
        }
    }
}
